...
function isInsideCircle(circle, point) {
  const d = (point.x - circle.center.x) * (point.x - circle.center.x)
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const bl = isStartIncluded ? '[' : '(';
  const br = isEndIncluded ? ']' : ')';
  return a < b ? `${bl + a}, ${b + br}` : `${bl + b}, ${a + br}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = ccn.toString().length - 1; i >= 0; i -= 1) {
    let digit = parseInt(ccn.toString().charAt(i), 10);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (b[str[i]]) {
      s.push(str[i]);
    } else if (Object.values(b).includes(str[i]) && str[i] !== b[s.pop()]) {
      return false;
    }
  }
